...
const pushTranslationSources = async (translationSourcesPath) => {
  ...
  if (!existsSync(translationSourcesPath)) {
    console.warn('Warning: the translations sources file does not exist. Please make sure you specify a valid path and make sure you run the translations:extract yarn command before.')
    return;
  }
  const url = `${process.env.DRUPAL_BASE_URL}${process.env.DRUPAL_CREATE_TRANSLATIONS_SOURCE_PATH}`;
  const translationSources = readFileSync(translationSourcesPath, {encoding: 'utf-8'});
  try {
    await axios.post(
      url,
      translationSources,
