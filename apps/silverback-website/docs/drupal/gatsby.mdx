...
- Taxonomy tags and media images used on the Article content.
...
- [apps/silverback-drupal-graphql-v3](https://github.com/AmazeeLabs/silverback-mono/tree/development/apps/silverback-drupal-graphql-v3)
  ...
  - Uses Drupal's Gatsby module to ping Gatsby Preview refresh and
    Gatsby Site rebuild.
...
- On Lagoon setup it is built in a Lagoon container and then is deployed to Netlify.
- Both implementations expose `{customAppUrl}/__rebuild` endpoint to re-build the site. The Gatsby data is preserved between the re-runs, so the re-builds are fast.
...
- Lagoon + Netlify setup can be found in the example.
  - Netlify could be replaced with any other static file server, including Lagoon itself.
- Anything that can run Node.js.

## How to run the example locally

Set up the monorepo locally as described in the [readme](https://github.com/AmazeeLabs/silverback-mono#setup-locally).
