...
- 🚚 Drupal content updates trigger Gatsby Preview refresh and Gatsby Site
  rebuild.
- 🏗 Gatsby reacts to that and fetches the content from Drupal and rebuilds the
  affected pages.
...
- 👋 Drupal editors are informed when Gatsby Preview is refreshed (via Toolbar).
...
You'll need a usual set of command line tools: Git, Yarn, Composer. Plus one
which is not that common: [direnv](https://direnv.net/docs/installation.html)
(don't forget about the [hook installation](https://direnv.net/docs/hook.html)!)

Prepare:

```shell
git clone https://github.com/AmazeeLabs/silverback-mono
cd silverback-mono
yarn
