...
These packages have been tried and we decided not to use them.

* `tsconfig-paths`: Typescript allows defining path aliases in `tsconfig.json`
  that helps to load typings with aliases. With plugins for webpack and jest
  its even possible to have full package aliases to avoid ugly imports like
  `../../../hooks` or similar. Imports are managed by the IDE anyway and
  integrations for all build systems are a heavy burden.
...
**Description**: react-intl has a set of [React components](https://formatjs.io/docs/react-intl/components) that provide a declarative way to setup an i18n context and format dates, numbers, and strings for display in a web UI.

**Features**:

- use[`format.js`](https://formatjs.io/) under the hood
- backed by Yahoo
- supports more than 150 languages globally
- uses the React context and higher-order components
- easy to use react components
- use arguments in your translated text
- localize date, time, numbers and currencies to different locales
- Pluralization