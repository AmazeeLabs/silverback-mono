...
1. You'll need at least [mglaman/phpstan-drupal](https://github.com/mglaman/phpstan-drupal) and [phpstan/extension-installer](https://github.com/phpstan/extension-installer) as dev dependencies for your module.

  Unfortunately, you cannot just add them to `require-dev` of your module `composer.json`. Because you will run tests for the module from within a Drupal installation and `composer install` will only load `require-dev` dependencies for the root project. Therefore, you need to keep your dev dependencies in a separate `-dev` project and then require this project from your testing Drupal installation.

  As an example, see Cypress module ([PR #274](https://github.com/AmazeeLabs/silverback-mono/pull/274)):
    - all dev dependencies are listed in `drupal/cypress-dev` metapackage
    - `drupal/cypress-dev` is required from `silverback-drupal` (which is used to run tests for `cypress`)
    - `drupal/cypress-dev` is required from `cypress` module (just as a guide for developers looking for dev dependencies)
...
- [phpstan/phpstan-strict-rules](https://github.com/phpstan/phpstan-strict-rules) provides very strict rules
