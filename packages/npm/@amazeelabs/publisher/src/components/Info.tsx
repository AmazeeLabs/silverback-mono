...
function History({
                   historyItems,
                 }: OrganismProps<{ historyItems: Array<{ id: number; startedAt: number; finishedAt: number; success: boolean; type: string; }> }>) {
      ...
      <div className="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
        <div className="overflow-hidden">
          <table className="min-w-full divide-y-2 divide-blue-900">
            <thead>
            <tr>
              <th scope="col" className="py-3.5 pl-4 pr-3 text-left font-semibold">Id</th>
              <th scope="col" className="px-3 py-3.5 text-left font-semibold">Type</th>
              <th scope="col" className="px-3 py-3.5 text-left font-semibold">Date</th>
              <th scope="col" className="px-3 py-3.5 text-left font-semibold">Status</th>
              <th scope="col" className="px-3 py-3.5 text-left font-semibold">Log</th>
            </tr>
            </thead>
            ...
            {historyItems.map((item) => {
              ...
              return (
                <>
                  <tr key={item.id} onClick={() => setExpanded(item.id)} className={'cursor-pointer'}>
                    <td className="whitespace-nowrap px-3 py-4">
                      {item.id}
                    </td>
                    <td className="whitespace-nowrap px-3 py-4 capitalize">{item.type}</td>
                    <td className="relative whitespace-nowrap py-4 pl-3 pr-4 sm:pr-6">
                      {date.toUTCString()},{' '}
                      {Math.round((item.finishedAt - item.startedAt) / 1000)} sec{' '}
                    </td>
                    <td className="whitespace-nowrap px-3 py-4">
                    <span className={clsx('inline-flex rounded-full px-2 text-sm leading-5 font-semibold',
                      {
                        'bg-green-100 text-green-500': item.success == true,
                    ...
                    <td className="whitespace-nowrap px-3 py-4">
                      <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" fill="currentColor"
                           className="ml-2.5 inline -mt-0.5" viewBox="0 0 16 16">
                  ...
                  <tr className={'!border-0 p-0 m-0'}>
                    <td colSpan={5} className={'pre-container p-0 m-0 !border-0'}>
  ...
  return (
    <div>
      <pre>
        {value
          ? (
            JSON.parse(value.logs) as Array<SpawnChunk & { timestamp: number }>
          )
            .map((item) => item.chunk)
            .join('\n')
          : ''}
      ...
      <div className={'max-w-full bg-gray-900 pb-10'}>
        <div className={'bg-gray-200 pt-6 pb-[9.5rem]'}>
          <div className="flex justify-between max-w-screen-2xl mx-auto px-4 md:px-6 lg:px-10 xl:px-14">
            ...
            <a onClick={scrollToBuildHistory}
               className={'shrink-0 font-alt text-yellow-500 text-sm font-bold uppercase cursor-pointer hover:text-white focus:text-white focus:outline-none transition'}>
              Build History
              <svg className="ml-2 inline -mt-0.5" xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                   fill="currentColor"
                   viewBox="0 0 16 16">
                <path fillRule="evenodd"
        ...
        <div className={'md:flex mb-8 px-4 md:px-6 lg:px-10 xl:px-14 max-w-screen-2xl mx-auto'}>
          <div className={'mr-4 w-full bg-white p-6 rounded-md -mt-[7.4rem] shadow-sm'}>
            <h2 className={'font-alt text-2xl font-bold text-blue-500 uppercase mb-1'}>Initial Build</h2>
            <h3 className={'text-xl mb-6'}>
              Gateway: <GatewayStatus/>{' '}
              {status.gateway === GatewayState.Ready ? (
                <a href="/" className={'text-blue-500 shadow-[inset_0_-1px_0_0_rgba(96,131,155,1)]'}>Go to website</a>
              ) : (
          ...
          <div className={'ml-4 w-full bg-white p-6 rounded-md -mt-[7.4rem] shadow-sm'}>
            <h2 className={'font-alt text-2xl font-bold text-blue-500 uppercase mb-1'}>
              Current Build
            </h2>
            <h3 className={'text-xl mb-6'}>
              <BuilderStatus/>
        ...
        <div className={'px-4 md:px-6 lg:px-10 xl:px-14 max-w-screen-2xl mx-auto'}>
          <div className={'bg-white shadow-sm p-6 rounded-md pre-container'} id="build-history">
            <h2 className={'font-alt text-2xl font-bold mb-2 text-blue-500 uppercase'}>Build history</h2>
            <div>
