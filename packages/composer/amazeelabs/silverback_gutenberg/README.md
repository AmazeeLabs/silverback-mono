...
- Add `Silverback:` prefixed matchers to the profile
  ...
  - To use a different profile when using the LinkControl component, add the machine name of the profile to the `subtype` query parameter in the component prop `suggestionsQuery` like below, where the custom linkit profile is called `customer`.
  ```
  <DrupalLinkControl
