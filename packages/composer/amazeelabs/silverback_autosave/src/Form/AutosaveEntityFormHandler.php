...
use Drupal\silverback_autosave\Ajax\OpenAutosaveDisabledDialog;
...
class AutosaveEntityFormHandler implements AutosaveEntityFormHandlerInterface, EntityHandlerInterface {
  ...
  public static function createInstance(ContainerInterface $container, EntityTypeInterface $entity_type) {
    return new static(
      ...
      $container->get('silverback_autosave.entity_form_storage'),
  ...
  public function autosaveFormAjax($form, FormStateInterface $form_state) {
    ...
    if ($timestamp == 'entity_saved_meanwhile') {
      ...
      if (is_numeric($input['silverback_autosave_last_autosave_timestamp']) || empty($input['silverback_autosave_last_autosave_timestamp'])) {
        // Alert the user about turning off autosave and deleting all the
        // autosaved states.
        $message = $this->configFactory->get('silverback_autosave.messages')
        ...
        $response->addCommand(new InvokeCommand('input[name="silverback_autosave_last_autosave_timestamp"]', 'attr', ['value', (string) $this->t('Autosave is turned off.')]));
  ...
  public static function entityFormEntityBuild($entity_type, EntityInterface $entity, array $form, FormStateInterface $form_state) {
    if ($silverback_autosave_session_id = static::getAutosaveFormSessionID($form_state)) {
      $entity->{static::AUTOSAVE_SESSION_ID} = $silverback_autosave_session_id;
    }
  }
  ...
  public function isAutosaveSubmitValid(FormStateInterface $form_state) {
    [$form_id, $entity] = $this->getFormIDandEntity($form_state);
    ...
    if ($entity instanceof EntityChangedInterface) {
      ...
      $input = $form_state->getUserInput();
      $changed_form_value = $input['changed'] ?? NULL;
      ...
      if (!$this->conflictEnabled && ($unchanged = $this->entityStorage->loadUnchanged($entity->id())) && ($unchanged->getChangedTimeAcrossTranslations() > $entity->getChangedTimeAcrossTranslations())) {
        $form_state->setTemporaryValue('silverback_autosave_last_autosave_timestamp', 'entity_saved_meanwhile');
        return FALSE;
      }
      else {
   ...
   * {@inheritdoc}
   */
  public function getLastAutosavedFormState(FormStateInterface $form_state, $silverback_autosave_session_id, $uid) {
    [$form_id, $entity] = $this->getFormIDandEntity($form_state);
    return $this->autosaveEntityFormStorage->getFormState($form_id, $entity->getEntityTypeId(), $entity->id(), $entity->language()->getId(), $uid, $silverback_autosave_session_id);
  }

  /**
   * {@inheritdoc}
   */
  public function storeState(FormStateInterface $form_state, $silverback_autosave_session_id, $autosave_timestamp, $uid) {
    [$form_id, $entity] = $this->getFormIDandEntity($form_state);
    $this->autosaveEntityFormStorage->storeEntityAndFormState($form_id, $silverback_autosave_session_id, $entity->getEntityTypeId(), $entity->id(), $entity->language()->getId(), $uid, $autosave_timestamp, $entity, $form_state);
  }
